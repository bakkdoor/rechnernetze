...
client_connection_t * connection_setup(const char * server_hostname, const char * server_port, char * username)
{
  ...
  hint.ai_protocol = 0; /* any */
  hint.ai_flags = 0;
  
  if (getaddrinfo(server_hostname, server_port, &hint, &cli_conn->server_addr_info) != 0) {
    error(true, "Could not resolv hostname '%s'\n", server_hostname);
  }
  ...
  memcpy(cli_conn->server_addr, cli_conn->server_addr_info->ai_addr, sizeof(struct sockaddr_in));
  
  printf("hostname: %s, ip: %s, port: %u\n", 
  cli_conn->server_addr_info->ai_canonname,
  inet_ntoa(cli_conn->server_addr->sin_addr),
  ntohs(cli_conn->server_addr->sin_port));
  return NULL;
